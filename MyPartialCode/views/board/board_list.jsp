			 ...
			 -->
	          ...
	          <table class="boardTable">
					...
					<tr class="boardTd">
						...
						<td class="ttitle1">
							<a class="board_font" href="http://localhost:9000/projectH/board_content.do?no=<%=vo.getNo()%>&rno=<%=vo.getRno()%>"
								onclick="window.open(this.href, '_blanck', 'width=1200, height=800'); return false">
								<%=vo.getTitle() %></a>  
								
						</td>				
						<td class="twriter1"><%=vo.getBname() %></td>		
						<td class="tbdate1"><%=vo.getBdate() %></td>
				...
				<c:if test="${pageVO.totPage>1}">
					<div class="paging">
						<c:forEach var="i" begin="${pageVO.pageStart}" end="${pageVO.pageEnd}" step="1">
							<c:url var="pageLink" value="board.do">
